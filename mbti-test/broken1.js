...
class Category
{
  ...
  askQuestion(trait)
  {
    ...
    ctx.font = '50px Arial';
    ctx.fillText('points' + Category.points+ ' ' + biggie,canvas.width-200, canvas.height-40);
...
function generateResult()
{
  ...
  ctx.textAlign = 'center';
  ctx.font = '50px Arial';
  ctx.fillText(query[0].score+ " " + query[1].score+ " " + query[2].score+ " " + query[3].score,canvas.width/2, canvas.height/2 + 60);
  getTrait();
}
function getTrait()
{
  let result = ['a', 'b', 'c', 'd'];
  if(query[0].score>50) result[0] = 'E';
  else result[0] = 'I';
  if(query[1].score>50) result[1] = 'S';
  else result[1] = 'N';
  if(query[2].score>50) result[2] = 'F';
  else result[2] = 'T';
  if(query[3].score>50) result[3] = 'J';
  else result[3] = 'P';
...
ctx.fillText('Created by Sergiu \'Old\' Batrin', canvas.width/2, canvas.height/2);
  ...
  if(e.key === ' ' && qnum<=40)
  {
    ...
    else
    {
      for(let i=0;i<=4;i++)
        if(c[i].info.chosen)
        {
          biggie = (query[currentTrait].index-1)%2;
